...
while True:
    ...
    if fake_seconds == 30:
        db.child('SPSDevices/device_distance').set(num)
        starting_num = num
    if fake_seconds >= 30:
        db.child('SPSDevices/device_distance').set(num)

        if abs(starting_num - int(num)) >= 200:
            email = smtplib.SMTP('smtp.gmail.com', 587)
            email.ehlo()
            email.starttls()
            emailadd = "databasehost69@gmail.com"
            password = "123456database"
            email.login(emailadd, password)

            #Get date of incident
            now = datetime.datetime.now()


            #Get the user we want to send the email to
            all_users = db.child("users/").get()
            # user we want
            wanted_user = None
            for i in all_users.val():
                user_items = []
                for val in db.child("users/" + str(i) + "/").get():
                    user_items.append(val.val())
                    # serialnumber is in the user's items or information
                if serialNum in user_items:
                    wanted_user = user_items
                    break

            email_to_send_to = ""
            for item in wanted_user:
                if ".com" in item or ".edu" in item:
                    email_to_send_to = item

            email.sendmail(emailadd, email_to_send_to, "Someone entered your room on \n" + now.strftime("%Y-%m-%d %H:%M"))
            print("EMAIL SENT!")
            time.sleep(30)
    print(num)
